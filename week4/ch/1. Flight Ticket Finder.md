[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-718a45dd9cf7e7f842a935f5ebbe5719a5e09af4491e668f4dbf3b35d5cca122.svg)](https://classroom.github.com/online_ide?assignment_repo_id=13925740&assignment_repo_type=AssignmentRepo)
...
`generateTravel`: adalah function untuk mengubah format array multi dimensi menjadi format array of object dari data.
...
```js
contoh: function generateTravel(passengersData) {
  // your code here
}
...
`getTicket`: adalah function untuk menghitung besaran total harga ticket yang dipesan oleh travel agent tersebut berdasarkan banyaknya passenger.
...
`getReward`: adalah function untuk untuk menghitung reward yang akan didapatkan oleh travel agent rekanan.
...
```js
contoh: function getReward(travelsData, ticketsBook) {
  // your code here
}
...
`travelAgentReward`: adalah main function yang akan memanggil function yang lain dan memanfaatkan hasil dari pemanggilan function.
...
- Bila parameter `travels` berupa array kosong, maka tampilkan `Data is empty`
- Bila parameter `travels` tidak ditemukan (undefined), maka tampilkan `Invalid data`
...
console.log(travelAgentReward(data, tickets));
// 'Invalid data'
